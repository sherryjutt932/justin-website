...
if (!isMobile() && !isTab()) {
  let xposition =
    aboutRef.offsetWidth - (window.innerWidth - (paddingsize + 126));
  aboutusTl
    .to(
      aboutRef,
      {
...
const about_icon_floatM = document.getElementsByClassName("about-icon-floatM");

const animateabouticonlistM = () => {
  if (about_icon_floatM[0]) {
    gsap
      .timeline()
      .to(
        [about_icon_floatM[0], about_icon_floatM[2]],
        3,
        //
        {
      ...
      .to(
        [about_icon_floatM[1], about_icon_floatM[3]],
        3,
        //
        {
  ...
  requestAnimationFrame(animateabouticonlistM);
};

if (isMobile()) {
  // Delay the animation function to ensure ScrollTrigger is ready
  setTimeout(() => {
    animateabouticonlistM();
  }, 100);
}

const animateabouticonlistT = () => {
  gsap.fromTo(
    ".about-icon-float",
    {
      yPercent: -30,
